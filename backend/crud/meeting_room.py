...
from backend.models.meeting_room import MeetingRoom
from backend.schemas.meeting_room import MeetingRoomCreate, MeetingRoomUpdate
...
async def get_meeting_room_by_id(
    ...
    print(type(session))
    print(dir(session))
    db_room = await session.get(MeetingRoom, id)
    return db_room
